...
// Usar directorio público para que persista mejor
...
export const trollsDB = {
  ...
  async delete(id) {
    try {
      console.log('🗑️ DB DELETE - Intentando eliminar troll con ID:', id, 'tipo:', typeof id);
      const data = readDatabase();
      
      console.log('📋 DB DELETE - Trolls en base de datos:', data.trolls.map(t => ({ id: t.id, nick: t.nick, tipo: typeof t.id })));
      
      // Convertir ID a número para comparación consistente
      const numericId = parseInt(id);
      console.log('🔢 DB DELETE - ID convertido a número:', numericId);
      
      const trollIndex = data.trolls.findIndex(t => {
        const trollId = parseInt(t.id);
        console.log(`🔍 DB DELETE - Comparando: ${trollId} === ${numericId} = ${trollId === numericId}`);
        return trollId === numericId;
      });
      
      if (trollIndex === -1) {
        console.log('❌ DB DELETE - Troll no encontrado con ID:', id);
        console.log('📋 DB DELETE - IDs disponibles:', data.trolls.map(t => ({ id: t.id, nick: t.nick })));
        return false;
      }
      
      const trollToDelete = data.trolls[trollIndex];
      console.log('🎯 DB DELETE - Troll encontrado para eliminar:', trollToDelete);
      
      data.trolls.splice(trollIndex, 1);
      writeDatabase(data);
      
      console.log('✅ DB DELETE - Troll eliminado exitosamente del JSON:', trollToDelete.nick);
      return true;
    } catch (error) {
      console.error('❌ DB DELETE - Error eliminando troll:', error);
      throw error;
    }
  },
