...
export const trollsDB = {
  ...
  async delete(id) {
    try {
      console.log('🗑️ Intentando eliminar troll con ID:', id, 'tipo:', typeof id);
      const data = readDatabase();
      
      console.log('📋 Trolls en base de datos:', data.trolls.map(t => ({ id: t.id, nick: t.nick, tipo: typeof t.id })));
      ...
      const trollIndex = data.trolls.findIndex(t => {
        ...
        console.log(`🔍 Comparando: ${trollId} === ${numericId} = ${trollId === numericId}`);
        return trollId === numericId;
      });
      
      if (trollIndex === -1) {
        console.log('❌ Troll no encontrado con ID:', id);
        console.log('📋 IDs disponibles:', data.trolls.map(t => ({ id: t.id, nick: t.nick })));
        return false;
      }
      
      const trollToDelete = data.trolls[trollIndex];
      console.log('🎯 Troll encontrado para eliminar:', trollToDelete);
      
      data.trolls.splice(trollIndex, 1);
      writeDatabase(data);
      
      console.log('✅ Troll eliminado exitosamente:', trollToDelete.nick);
      return true;
    } catch (error) {
